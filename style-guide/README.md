...
- Используйте полный вариант открывающего тега **`<?php`** в файлах, содержащих только код PHP.

- Используйте короткий открывающий тег **`<?`** в файлах, содержащих код HTML. Вместо **`<? echo $var; ?>`** используйте краткую запись **`<?= $var; ?>`**:
...
Прочитайте про особенности использования альтернативного синтаксиса в [документации](http://php.net/manual/ru/control-structures.alternative-syntax.php).
...
- Не используйте знак подчеркивания вначале имени свойства или метода для обозначения **`protected`** или **`private`**.

- Ключевое слово **`static`** должно быть после указания видимости.

- Указание **`abstract`** и **`final`** должны быть перед указанием видимости.
